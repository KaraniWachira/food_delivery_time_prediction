...
models <- list(
  lm = lm, # linear model
  rpart = train, # decision trees model
  rf = train,  # random forest model
  xgbTree = train #XGBoost model
)

# Define the parameter grid
param_grid <- list(
  lm = list(),
  rpart = list(maxdepth = c(3, 5, 7)),
  rf = list(ntree = c(100, 200, 300)),
  xgbTree = list(nrounds = c(20, 25, 30), max_depth = c(5, 7, 9))
)
...
for (i in seq_along(models)) {
  ...
  grid_search <- train(
    x = X_train,
    y = y_train,
    method = model_name,  # Corrected this line
    tuneGrid = param_grid[[model_name]],
    trControl = ctrl,
    metric = "R2"
  )
  
  # Store results
  best_params[[model_name]] <- grid_search$bestTune
  best_r2_scores[[model_name]] <- grid_search$results$R2
  
  cat(paste0(model_name, ":\n"))
  cat("Best parameters:", best_params[[model_name]], "\n")
  cat("Best R2 score:", max(best_r2_scores[[model_name]]), "\n\n")
}
