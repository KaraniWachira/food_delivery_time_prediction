...
# Identify non-categorical numeric predictors (excluding dates)
non_cat_numeric_predictors <- setdiff(numeric_predictors, c("Order_Date", "Time_Order_picked"))
...
preprocess_recipe <- recipe(Time_taken_min ~ ., data = train_data) %>%
  step_scale(all_of(non_cat_numeric_predictors)) %>% 
  step_center(all_of(non_cat_numeric_predictors)) %>%
  step_dummy(all_nominal(), -all_numeric())  # Treat categorical variables as dummy variables
