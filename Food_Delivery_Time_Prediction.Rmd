...
```{r}
# Label Encoding is being used due to the presence of multiple features with categorical values. assigns a unique numerical label to each category within a feature

label_encoding <- function(train_df) {
...
# Assuming train_df is a data frame in R
...
# Features (X)
X <- train_df[, -which(names(train_df) == "`Time_taken(min)`")]
...
# Create a preProcess object for standardization
preprocess <- preProcess(X_train,  method = c("center", "scale"))

# Apply the standardization transformation to the training data
X_train <- predict(preprocess, newdata = X_train)

# Apply the standardization transformation to the training data
X_test <- predict(preprocess, newdata = X_train)

# Apply the standardization transformation to the testing data
X_test <- predict(preprocess, newdata = X_test)



# This approach manually standardizes the data using the mean and standard deviation computed from the training set.
```
...
# Load your data (assuming X_train and y_train are data frames)

# Define models using parsnip
lm_model <- linear_reg()
tree_model <- decision_tree(cost_complexity = tune())
rf_model <- rand_forest(mtry = tune(), trees = tune())

# Define a recipe for preprocessing (adjust as needed)
recipe <- recipe(y_train ~ ., data = X_train) %>%
  # Add necessary preprocessing steps here, e.g.,
  # step_normalize(all_numeric())

# Create a resampling object for cross-validation
cv <- vfold_cv(X_train, v = 5)

# Create a workflow for each model
workflows <- list(
  lm_workflow = workflow() %>% add_model(lm_model) %>% add_recipe(recipe),
  tree_workflow = workflow() %>% add_model(tree_model) %>% add_recipe(recipe),
  rf_workflow = workflow() %>% add_model(rf_model) %>% add_recipe(recipe)
)

# Tune each model using tune_grid
tuned_models <- map(workflows, tune_grid, resamples = cv, grid = param_grid)

# Fit the best models
final_models <- map(tuned_models, select_best) %>%
  map(fit, X_train)

# Evaluate performance
results <- map(final_models, collect_metrics, metric = "rmse")

# Display results
for (i in seq_along(results)) {
  cat(names(final_models)[i], ":\n")
  print(results[[i]])
}
