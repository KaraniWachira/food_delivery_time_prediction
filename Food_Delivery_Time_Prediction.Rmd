...
train_df$`Time_taken(min)` <- as.integer(sapply(strsplit(train_df$`Time_taken(min)`, ' '), function(x) {
...
train_df$Delivery_person_Age <- as.numeric(train_df$Delivery_person_Age)

train_df$Delivery_person_Ratings <- as.numeric(train_df$Delivery_person_Ratings)

train_df$multiple_deliveries <- as.numeric(train_df$multiple_deliveries)

# Update datatype from character to Date ##ORDER DATE WRONG
...
## check for any duplicate values in the dataset to ensure data accuracy and eliminate bias
## If duplicates are found, I will need to remove them to maintain the integrity of the data
...
calculate_time_diff <- function(df) {
  # Convert columns to time duration
  df$Time_Orderd <- as.duration(df$Time_Orderd)
  df$Time_Order_picked <- as.duration(df$Time_Order_picked)
  
  # Print intermediate results for debugging
  print("After duration conversion:")
  print(df)
  
  # Calculate formatted times
  df$Time_Order_picked_formatted <- df$Order_Date + ifelse(df$Time_Order_picked < df$Time_Orderd, days(1), days(0)) + df$Time_Order_picked
  df$Time_Ordered_formatted <- df$Order_Date + df$Time_Orderd
  
  # Print intermediate results for debugging
  print("After formatted times calculation:")
  print(df)
  
  # Calculate order_prepare_time in minutes
  df$order_prepare_time <- as.numeric(df$Time_Order_picked_formatted - df$Time_Ordered_formatted) / 60
  
  # Print intermediate results for debugging
  print("After order_prepare_time calculation:")
  print(df)
  
  # Handle null values by filling with the median
  df$order_prepare_time[is.na(df$order_prepare_time)] <- median(df$order_prepare_time, na.rm = TRUE)
  
  # Drop columns
  df <- df %>%
    select(-c("Time_Orderd", "Time_Order_picked", "Time_Ordered_formatted", "Time_Order_picked_formatted", "Order_Date"))
  
  return(df)
}

# Assuming train_df is your data frame
train_df <- calculate_time_diff(train_df)
head(train_df)
...
#### Map showing the different delivery points from the restaurant the meals were ordered from
...
center <- c(mean(data_frame$Restaurant_longitude), mean(data_frame$Restaurant_latitude))
map <- get_map(center, zoom = 12)

# Create the scatter plot on top of the map
ggmap(map) +
  geom_point(
    aes(x = Longitude, y = Latitude),
    data = data_frame,
    color = "red",  # Customize color if desired
    size = 4     # Customize point size if desired
  )

```
#### Data Preprocessing 

## Catergorical feature encoding to transform categorical values into numerical representations.
```{r}
library(recipes)
library(dplyr)
...
# Specify target role (optional for encoding, but recommended for clarity)
rec <- rec %>% update_role(`Time_taken(min)`, new_role = "outcome")

# Select only categorical features
rec <- rec %>% step_select(all_nominal())

# Apply one-hot encoding
rec <- rec %>% step_dummy(all_nominal())

# Prepare and bake the recipe
rec_prep <- prep(rec, data = train_df)
df_encoded <- bake(rec_prep, new_data = train_df)
...
df_train <- read.csv("data/train.csv")  # Replace with your actual data file

# Split the data with initial_split
df_split <- initial_split(df_train, prop = 0.8)

# Extract training and testing sets
train_df <- training(df_split)
test_df  <- testing(df_split)

# Prepare the recipe (assuming your existing recipe is stored as "rec")
rec_prep <- prep(rec, training = train_df)

# Apply the prepared recipe to both sets
train_data <- juice(rec_prep)  # Apply to training data directly
test_data <- juice(rec_prep, new_data = test_df)  # Apply to testing data
...
## To address the issue of varying scales in the food delivery dataset.It will transform the features to have a zero mean and a unit standard deviation by doing so, it ensure that no single feature dominates the learning process and enable a fair comparison among the features.
## Standardization facilitates faster convergence and enhances the overall performance of the machine learning model.
...
## I will train a machine learning model to predict the food delivery time, which falls under the domain of regression problems since I aim to predict a numerical value

## To determine the most suitable algorithm for our data, we can use the Metrics function from Yardstick package which.
